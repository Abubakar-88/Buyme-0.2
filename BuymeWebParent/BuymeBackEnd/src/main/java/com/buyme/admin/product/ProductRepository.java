...
public interface ProductRepository extends PagingAndSortingRepository<Product, Integer> {
	...
	@Query("Update Product p SET p.averageRating = COALESCE((SELECT AVG(r.rating) FROM Review r WHERE r.product.id = ?1), 0),"
			...
			+ "WHERE p.id = ?1")
	@Modifying
	public void updatedReviewCountAndAverageRating(Integer productId);


	@Query("SELECT COUNT(c.id) FROM Product c WHERE c.enabled = true")
	Long countEnabledProduct();

	@Query("SELECT COUNT(c.id) FROM Product c WHERE c.enabled = false")
	Long countDisabledProduct();
